...
public record TimeBellFreezePayload(boolean frozen) implements CustomPacketPayload {
    ...
    public static void handle(final TimeBellFreezePayload payload, final ClientPlayNetworking.Context context) {
        context.client().execute(() -> TimeBellOverlay.setFrozen(payload.frozen()));
    }
