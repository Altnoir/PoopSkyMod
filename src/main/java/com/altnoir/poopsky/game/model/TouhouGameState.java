...
public final class TouhouGameState {
    ...
    public void applySnapshot(CompoundTag tag) {
        playerX = tag.getFloatOr("playerX", 0.0F);
        playerY = tag.getFloatOr("playerY", 0.0F);
        bossX = tag.getFloatOr("bossX", 0.0F);
        bossY = tag.getFloatOr("bossY", 0.0F);
        bossHp = tag.getIntOr("bossHp", 0);
        bossMaxHp = tag.getIntOr("bossMaxHp", 0);
        ...
        bossScale = tag.getFloatOr("bossScale", 1.0F);
        hasSpeedBoost = tag.getBooleanOr("hasSpeedBoost", false);
        hasDoubleShot = tag.getBooleanOr("hasDoubleShot", false);
        hasDing = tag.getBooleanOr("hasDing", false);

        bosses.clear();
        int bossCount = tag.getIntOr("bossCount", 0);
        if (bossCount > 0) {
            for (int i = 0; i < bossCount; i++) {
                ActiveBoss activeBoss = new ActiveBoss(
                        null,
                        tag.getFloatOr("bossX" + i, 0.0F),
                        tag.getFloatOr("bossY" + i, 0.0F),
                        tag.getIntOr("bossHp" + i, 0),
                        tag.getIntOr("bossMaxHp" + i, 0)
                );
                activeBoss.hitTicks = tag.getIntOr("bossHitTicks" + i, 0);
                bosses.add(activeBoss);
            }
        } else if (tag.contains("bossX")) {
            bosses.add(new ActiveBoss(
                    null,
                    tag.getFloatOr("bossX", 0.0F),
                    tag.getFloatOr("bossY", 0.0F),
                    tag.getIntOr("bossHp", 0),
                    tag.getIntOr("bossMaxHp", 0)
            ));
        }

        powerUps.clear();
        int powerUpCount = tag.getIntOr("powerUpCount", 0);
        for (int i = 0; i < powerUpCount; i++) {
            powerUps.add(new PowerUp(
                    tag.getFloatOr("powerUpX" + i, 0.0F),
                    tag.getFloatOr("powerUpY" + i, 0.0F),
                    tag.getFloatOr("powerUpVX" + i, 0.0F),
                    tag.getFloatOr("powerUpVY" + i, 0.0F),
                    PowerUpType.valueOf(tag.getStringOr("powerUpType" + i, PowerUpType.SPEED.name())),
                    tag.getIntOr("powerUpLife" + i, 0)
            ));
        }

        readBullets(enemyBullets, tag.getIntArray("enemyBullets").orElseGet(() -> new int[0]));
        readBullets(playerBullets, tag.getIntArray("playerBullets").orElseGet(() -> new int[0]));
    }
