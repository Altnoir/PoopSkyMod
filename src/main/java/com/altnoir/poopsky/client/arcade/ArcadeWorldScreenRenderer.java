...
public final class ArcadeWorldScreenRenderer {
    ...
    public static void setGame(BlockPos pos, GameDiscItem disc, Game game) {
        ScreenState state = getOrCreate(pos, disc);
        state.activeGame = game;
        state.ensureResources();
    }

    public static void clearGame(BlockPos pos, Game game) {
        ScreenState state = SCREENS.get(pos);
        if (state != null && state.activeGame == game) {
    ...
    private static void renderFrame() {
        ...
        for (ScreenState state : SCREENS.values()) {
            if (state.activeGame == null && state.cartridgeGame == null) {
                continue;
            }
            state.ensureResources();
            renderState(state);
