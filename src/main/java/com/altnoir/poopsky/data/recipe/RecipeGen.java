...
public class RecipeGen extends FabricRecipeProvider {
    ...
    public RecipeGen(FabricDataOutput output, CompletableFuture<HolderLookup.Provider> provider) {
    ...
    private void buildCustomRecipes(RecipeOutput recipeOutput) {
        ...
        RecipeOutput fd = withConditions(recipeOutput, modLoaded(PoMods.FARMERSDELIGHT.id()));
    ...
    private void buildSieveRecipes(RecipeOutput recipeOutput) {
        RecipeOutput createNotLoaded = withConditions(recipeOutput, not(modLoaded(PoMods.CREATE.id())));
        RecipeOutput fdLoaded = withConditions(recipeOutput, modLoaded(PoMods.FARMERSDELIGHT.id()));
    ...
    private void buildFlyBarrelRecipes(RecipeOutput recipeOutput) {
        RecipeOutput create = withConditions(recipeOutput, modLoaded(PoMods.CREATE.id()));
        RecipeOutput ae2 = withConditions(recipeOutput, modLoaded(PoMods.AE2.id()));
        RecipeOutput mekanism = withConditions(recipeOutput, modLoaded(PoMods.MEKANISM.id()));
    ...
    private void spallEnchantedRecipe(RecipeOutput recipeOutput, Holder<Enchantment> enchantment, ItemLike item, RecipeCategory category, String... patterns) {
        ...
        RecipeOutput enchantedOutput = new RecipeOutput() {
            @Override
            public void accept(ResourceLocation id, Recipe<?> recipe, AdvancementHolder advancement) {
                if (!(recipe instanceof ShapedRecipe shapedRecipe)) {
                    throw new IllegalStateException("Expected shaped recipe for " + id);
                }
                recipeOutput.accept(id, new ShapedRecipe(
                        shapedRecipe.getGroup(), shapedRecipe.category(), shapedRecipe.pattern,
                        stack, shapedRecipe.showNotification()), advancement);
            }
            ...
            public ResourceLocation getRecipeIdentifier(ResourceLocation recipeId) {
                return recipeOutput.getRecipeIdentifier(recipeId);
            }
        };
        ShapedRecipeBuilder builder = ShapedRecipeBuilder.shaped(category, item);
        ...
        builder.define('M', PoItems.SPALL)
                .define('S', Items.STICK)
                .unlockedBy(getItemName(PoItems.SPALL), has(PoItems.SPALL))
                .save(enchantedOutput);
    }

    private static ResourceCondition modLoaded(String modId) {
        return ResourceConditions.allModsLoaded(modId);
    }

    private static ResourceCondition not(ResourceCondition condition) {
        return ResourceConditions.not(condition);
    }
