...
public class PopTntMinecartItem extends Item {
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (!(level instanceof ServerLevel serverLevel)) return InteractionResult.sidedSuccess(true);
        BlockPos pos = context.getClickedPos().relative(context.getClickedFace());
        if (!serverLevel.getBlockState(context.getClickedPos()).is(net.minecraft.world.level.block.Blocks.RAIL)) {
            return InteractionResult.PASS;
        }
        var cart = PoEntityType.POP_TNT_MINECART.get().create(serverLevel);
        if (cart == null) return InteractionResult.PASS;
        cart.moveTo(pos.getX() + 0.5, pos.getY() + 0.0625, pos.getZ() + 0.5,
