...
public class Toilet extends BlockWithEntity implements Portal{
    ...
    protected void appendProperties(StateManager.Builder<Block, BlockState> builder) {
        builder.add(Properties.HORIZONTAL_FACING, AXIS);
    }

    @Override
    public void onLandedUpon(World world, BlockState state, BlockPos pos, Entity entity, float fallDistance) {
        if (entity.canUsePortals(false) && !world.isClient && fallDistance >= 1.0F) {
    ...
    public TeleportTarget createTeleportTarget(ServerWorld world, Entity entity, BlockPos pos) {
        if (world.getBlockEntity(pos) instanceof ToiletBlockEntity toilet) {
            ...
            Vec3d velocity = getTeleportVelocity(entity);
    ...
    private static Vec3d getTeleportVelocity(Entity entity) {
        return new Vec3d(entity.getVelocity().x, -entity.getVelocity().y, entity.getVelocity().z);
    }
    @Override
    ...
    private void onPoop(World world, Entity entity) {
        ...
        world.playSound(null, entity.getX(), entity.getY(), entity.getZ(), PSSounds.FART, entity.getSoundCategory(), 1.0F, pitch);
        world.syncWorldEvent(2001,
                BlockPos.ofFloored(
                        entity.getX(),
                        entity.getY() + 0.1,
                        entity.getZ()
